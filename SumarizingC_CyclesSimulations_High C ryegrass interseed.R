...
setwd("C:/Felipe/OrganicTransitions_N2OROSE/CyclesSimulation/RoseRCodeScripts/SumarizingC_CyclesSimulations/ROSE Simulation Work/High C ryegrass interseed") ;

# Create a directory for sumarizing outputs

# dir.create()

Scenario.name<-"High C ryegrass interseed"
...
#  i=list.files()[2]
 ...
 for (i in list.files()) {
     ...
     Summary<-rbind(Summary,Nleach.cum)    ;
     
     Data_to_Remove<-ls()[!ls() %in% c("Summary","i")]
...
Summary$Start.year<-as.numeric(format(as.Date(substr(Summary$File_Name,start=nchar(Summary$File_Name)-4, stop=nchar(Summary$File_Name)-3),format="%y"), "%Y")  ) ;

Summary<-Summary[order(Summary$Start.year,Summary$DATE),]   ;

Summary$DATE<-format(Summary$DATE,"%m/%d/%Y")    ;

Summary<-Summary[,c("Start.year", "DATE", "NO3.LEACHING", "File_Name")]      ;

Summary.Rotation<-Summary[seq(1,dim(Summary)[1],2),]     ;


writeWorksheetToFile("../OutputSummary/Summary.xlsx",Summary.Rotation, sheet="High C ryegrass interseed" )      ; 
